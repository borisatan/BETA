...
const enforceCacheLimits = () => {
  ...
  while (sortedKeys.length > CACHE_CONFIG.maxEntries) {
    ...
    if (oldestKey) {
      console.log(`Evicting cache entry for ${oldestKey} due to cache size limits`);
...
const trimTransactionCache = (transactions: Transaction[]): Transaction[] => {
  ...
  console.log(`Trimming transaction cache from ${transactions.length} to ${CACHE_CONFIG.maxTransactions} items`);
...
const Dashboard = () => {
  ...
  const prepareCategoryData = (
    ...
    try {
      console.log(`Preparing category data from ${aggregations?.length || 0} aggregations`);
      ...
      if (aggregations && aggregations.length > 0) {
        for (const agg of aggregations) {
          ...
          if (agg.categoryId) {
            ...
            if (!categoryTotals[agg.categoryId]) categoryTotals[agg.categoryId] = 0;
            // Add totalExpenses to the category total
            categoryTotals[agg.categoryId] += agg.totalExpenses || 0;
            
            if (!categoryTransactionCounts[agg.categoryId]) categoryTransactionCounts[agg.categoryId] = 0;
            categoryTransactionCounts[agg.categoryId] += agg.transactionCount || 0;
          }
        }
      }
      
      // Get user categories to map IDs to names and logos
      CategoryService.getUserCategories(auth.currentUser?.uid || '').then((categories) => {
        ...
        if (summaries.length > 0) {
          setPieData(pieChartData.map(item => ({
            name: item.name,
            amount: item.value,
            color: getColorForIndex(pieChartData.indexOf(item)),
  ...
  const prepareCategorySummaryFromTransactions = async (
    ...
    if (viewMode === 'main') {
      mainCategories = await CategoryService.getUserMainCategories(auth.currentUser?.uid || '');
    ...
    console.log("Category lookup maps created - Categories:", Object.keys(categoryById).length, 
                "Main categories:", Object.keys(mainCategoryById).length);
    
    // Check for the actual structure of a transaction to help with debugging
    if (transactions.length > 0) {
      console.log("First transaction object structure:", JSON.stringify(transactions[0]));
    }
    ...
    const expenseTransactions = transactions.filter(t => {
      ...
      const hasExpenseType = t.transactionType === 'expense' || (t as any).type === 'expense';
    ...
    expenseTransactions.forEach(transaction => {
      ...
      const categoryIdOrName = transaction.categoryId || 
      ...
      if (categoryIdOrName) {
        ...
        if (!categoryById[categoryIdOrName] && categoryByName[categoryIdOrName.toLowerCase()]) {
        ...
        if (viewMode === 'subcategories') {
          ...
          if (categoryFound) {
            ...
            categoryTotals[categoryIdOrName] = (categoryTotals[categoryIdOrName] || 0) + amount;
            categoryTransactionCounts[categoryIdOrName] = (categoryTransactionCounts[categoryIdOrName] || 0) + 1;
          }
        } else {
          ...
          if (categoryFound && mainCategoryName) {
            ...
            mainCategoryTotals[mainCategoryName] = (mainCategoryTotals[mainCategoryName] || 0) + amount;
            mainCategoryTransactionCounts[mainCategoryName] = (mainCategoryTransactionCounts[mainCategoryName] || 0) + 1;
          } else {
            ...
            if (mainCategoryByName[lowerCaseName]) {
              mainCategoryTotals[mainCategoryByName[lowerCaseName].name] = 
                (mainCategoryTotals[mainCategoryByName[lowerCaseName].name] || 0) + amount;
              mainCategoryTransactionCounts[mainCategoryByName[lowerCaseName].name] = 
                (mainCategoryTransactionCounts[mainCategoryByName[lowerCaseName].name] || 0) + 1;
            } else {
    ...
    console.log("Uncategorized total:", uncategorizedTotal, "count:", uncategorizedCount);
    
    // Calculate total spending
    let categorizedSpending = 0;
    if (viewMode === 'subcategories') {
      categorizedSpending = Object.values(categoryTotals).reduce((sum, amount) => sum + amount, 0);
    } else {
      categorizedSpending = Object.values(mainCategoryTotals).reduce((sum, amount) => sum + amount, 0);
    }
    const totalSpending = categorizedSpending + uncategorizedTotal;
    
    console.log("Total spending calculated:", totalSpending, 
      "(Categorized:", categorizedSpending, "Uncategorized:", uncategorizedTotal, ")");
    ...
    if (viewMode === 'subcategories') {
      ...
      Object.keys(categoryTotals).forEach((categoryId, index) => {
        ...
        const matchedCategory = categoryById[categoryId] || 
      ...
      Object.keys(mainCategoryTotals).forEach((mainCategoryName, index) => {
        ...
        const matchedMainCategory = mainCategories.find(mc => 
          mc.name.toLowerCase() === mainCategoryName.toLowerCase()
        );
        ...
        if (matchedMainCategory) {
          summaries.push({
            ...
            transactionCount: mainCategoryTransactionCounts[mainCategoryName] || 0,
            amount: amount,
            percentage: totalSpending > 0 ? (amount / totalSpending) * 100 : 0,
            color: getColorForIndex(index),
          });
    ...
    if (uncategorizedCount > 0) {
      summaries.push({
        ...
        percentage: totalSpending > 0 ? (uncategorizedTotal / totalSpending) * 100 : 0,
        color: "#888888",
      });
    }
  ...
  const fetchTransactionsWithCache = async (
    ...
    try {
      ...
      const filteredTransactions = await TransactionService.getTransactionsByDateRange(
        userId,
        currentStart,
        currentEnd
      );
      
      console.log(`Retrieved ${filteredTransactions.length} transactions for ${timeFrame}`);
  ...
  const invalidateTransactionCache = (timeFrame?: "week" | "month" | "6months" | "year") => {
    const userId = auth.currentUser?.uid;
    if (!userId) return;
  ...
  useEffect(() => {
    console.log(`Timeframe changed to ${timeFrame}, fetching dashboard data...`);
    ...
    (async () => {
      try {
        ...
        if (PreloadService.getPreloadedTransactions(timeFrame)) {
          transactions = PreloadService.getPreloadedTransactions(timeFrame) as Transaction[];
          console.log(`Using ${transactions.length} preloaded transactions for category view update`);
        } else if (cache && cache.transactions.length > 0) {
          transactions = cache.transactions;
          console.log(`Using ${transactions.length} cached transactions for category view update`);
        } else {
          // Fallback to fetching if needed
          transactions = await fetchTransactionsWithCache(timeFrame);
          console.log(`Fetched ${transactions.length} transactions for category view update`);
        }
        
        if (transactions.length > 0) {
          const { categorySummaries: summaries, pieData: categoryData } = 
            await prepareCategorySummaryFromTransactions(transactions, categoryViewMode);
          
          console.log(`Updated category summaries for ${categoryViewMode} view with ${summaries.length} items`);
  ...
  const fetchDashboardData = async (
    ...
    console.log(`[Dashboard] Fetching dashboard data for timeframe: ${timeFrame}`);
    setIsLoading(true);
    ...
    try {
      ...
      if (preloadedTransactions && preloadedTransactions.length > 0 && !isRefreshing) {
        console.log(`[Dashboard] Using ${preloadedTransactions.length} preloaded transactions`);
        ...
        if (categoryViewMode === 'subcategories') {
          ...
          if (preloadedAggregations) {
            console.log(`[Dashboard] Using preloaded aggregations for ${timeFrame}`);
      ...
      // Fetch transactions for both periods
        ...
        console.log(`[Dashboard] Fetched ${currentTransactionsData.length} current and ${previousTransactionsData.length} previous transactions`);
      ...
      if (categoryViewMode === 'subcategories') {
        try {
          ...
          console.log(`[Dashboard] Fetched ${currentAggregations.length} current aggregations`);
          
          processAggregations(currentAggregations);
          
          // Cache the data for future use
          if (currentAggregations.length > 0 && currentTransactionsData.length > 0) {
  ...
  const processSpendingData = (aggregations: DailyAggregation[]): void => {
    try {
      ...
      const totalSpending = dataPoints.reduce((sum, point) => sum + point.amount, 0);
      const averageSpending = dataPoints.length > 0
  ...
  const processMainCategoryData = async (transactions: Transaction[]): Promise<void> => {
    try {
      ...
      // Create lookup maps
      const categoryMap = new Map(categories.map(cat => [cat.id, cat]));
      const mainCategoryMap = new Map(mainCategories.map(mainCat => [mainCat.id, mainCat]));
      
      // Create category-to-main-category mapping
      const categoryToMainCategory = new Map();
      categories.forEach(cat => {
        if (cat.mainCategory) { // Use mainCategory instead of mainCategoryId
      ...
      const expenseTransactions = transactions.filter(t => 
        t.transactionType === 'expense' && t.amount > 0
      );
  ...
  const CategorySummaryCard = () => {
    ...
    return (
      <ChartCard title={`Top Categories - ${categoryViewMode === 'subcategories' ? 'Subcategories' : 'Main Categories'}`}>
              ...
              {displayCategories.map((category) => (
                <View 
                  ...
                  className={`flex-row justify-between items-center py-3 ${
                    category.id !== displayCategories[displayCategories.length - 1].id 
              ...
              {categorySummaries.length > 5 && (
                ...
                >
                  <Text className={`font-medium ${isDarkMode ? "text-blue-400" : "text-blue-600"}`}>
                    {showAllCategories ? "Show Less" : `View All (${categorySummaries.length})`}
                  </Text>
                </TouchableOpacity>
              )}
  ...
  const calculateSpendingForTimeframe = (transactions: Transaction[], startDate: Date, endDate: Date) => {
    try {
  ...
  return (
    ...
    >
          ...
          <View className="relative">
            ...
            >
              <Text className={`mr-2 ${isDarkMode ? "text-white" : "text-gray-800"}`}>
                {categoryViewMode === 'subcategories' ? 'By Subcategories' : 'By Main Categories'}
              </Text>
              <MaterialIcons 
            ...
            {showCategoryDropdown && (
              ...
              >
                <TouchableOpacity
                  className={`px-4 py-3 ${categoryViewMode === 'subcategories' ? 
                ...
                <TouchableOpacity
                  className={`px-4 py-3 ${categoryViewMode === 'main' ? 
                    (isDarkMode ? "bg-gray-600" : "bg-gray-100") : ""}`}
                  onPress={() => {
      ...
      <View className="w-full items-center px-4">
        ...
        {(() => { console.log("Rendering CategorySummaryCard in the layout"); return null; })()}
        <CategorySummaryCard />
      </View>
